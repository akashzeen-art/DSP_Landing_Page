(function () {
  ...
  /**
   * Palestine Dual Operator — Z Game + Advertizer
   * UI: loaderlite (FileShare) — same as LP1_Dua_Palastine_Propellar
   *
   * Jawwal  (cid=3056) portal 570 — 1.16 NIS/day — unsub 0257 → 37799
   * Ooredoo (cid=3055) portal 569 — 1.5 NIS/day  — unsub 08 → 6976
   *
   * Flow: MSISDN → operator → sendpin → pin → verifypin → Advertizer → thankyou → portal
   * Postback amount: 1.16 (Jawwal) or 1.5 (Ooredoo)
   */
      ...
      getUserIp(function (ip) {
        ...
        callApi("verifypin", params)
          .then(function (resp) {
            ...
            persist("pb_amount", op.price || "1.16");
