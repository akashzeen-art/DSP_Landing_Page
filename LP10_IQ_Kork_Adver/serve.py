#!/usr/bin/env python3
"""
Iraq Korek ZD Know Your Country Advertizer LP local server.
  python3 serve.py
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "IQ-Korek-Advertizer-LocalProxy/1.0"}, method="GET")
    try:
...
def main() -> None:
    ...
    print("Iraq Korek ZD Know Your Country Advertizer LP: http://%s:%s/" % (HOST, PORT))
