(function () {
   ...
   * UI: LP10_LK_Mobitel_Prop Theme-496
   *
   * cid=19 — 168.144.122.72/prod/CGcmp/
   * 150 CFA / week — PIN 4
   * MSISDN: +242, local 06XXXXXXX (API sample 24206xxxxxxx)
   * Postback payout: 150
   */

  var msisdnFormat = /^06[0-9]{7}$/;
  var COUNTRY = "242";
  var PIN_LENGTH = 4;
  var PAYOUT = "150";
  var CID = "19";
  var API_HOST = "168.144.122.72";
  ...
  function normalizeLocal(raw) {
    ...
    /* 6XXXXXXX (8) → 06XXXXXXX (9) */
    if (digits.length === 8 && digits.charAt(0) === "6") digits = "0" + digits;
  ...
  function getUserIp(cb) {
    if (track("user_ip")) {
      if (typeof cb === "function") cb(track("user_ip"));
      return;
    }
    var done = false;
    function finish(ip) {
  ...
  function apiUrl(path, params) {
    ...
    return "http://" + API_HOST + "/prod/CGcmp/" + path + "?" + q.toString();
  }
  ...
  function portalUrl(msisdn) {
    return "http://" + API_HOST + "/prod/CGcmp/redirect?cid=" + CID + "&msisdn=" + encodeURIComponent(msisdn);
  }
  ...
  var t = {
    en: {
      ...
      disclaimer:
        "SelfieStar is a subscription service for MTN Congo at 150 CFA per week. Your subscription renews automatically until cancelled.",
      errmsg: {
        m: "Please enter your mobile number",
        o: "Please enter a valid Congo MTN number (06 followed by 7 digits).",
    ...
    fr: {
      heroTitle: "Vous êtes à un pas de regarder",
      ...
      disclaimer:
        "SelfieStar est un service d’abonnement MTN Congo à 150 FCFA par semaine. L’abonnement se renouvelle automatiquement jusqu’à résiliation.",
      errmsg: {
        m: "Veuillez entrer votre numéro mobile",
        o: "Veuillez entrer un numéro MTN Congo valide (06 suivi de 7 chiffres).",
