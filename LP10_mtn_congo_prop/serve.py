#!/usr/bin/env python3
"""
Congo MTN SelfieStar Propeller LP.
  python3 serve.py
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "CG-MTN-LocalProxy/1.0"}, method="GET")
    try:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def proxy_cg(self, parsed: urllib.parse.ParseResult) -> None:
        qs = urllib.parse.parse_qs(parsed.query, keep_blank_values=True)
        api_host = (qs.get("host") or ["168.144.122.72"])[0].strip()
        ...
        url = "http://%s/prod/CGcmp/%s%s" % (api_host, api_path, ("?" + query) if query else "")
    ...
    def proxy_propeller(self, parsed: urllib.parse.ParseResult) -> None:
        ...
        payout = ((qs.get("payout") or ["150"])[0]).strip() or "150"
...
def main() -> None:
    ...
    print("Congo MTN SelfieStar LP: http://%s:%s/" % (HOST, PORT))
