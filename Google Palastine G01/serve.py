#!/usr/bin/env python3
"""
Google Palestine G01 — local server (NO PHP required).

  cd "Google Palastine G01"
  python3 serve.py
  open http://127.0.0.1:8085/?clickid=TEST

Live: https://castmax-fun.com/psgog01?clickid={gclid}
  adid=232 | Ooredoo cmpid=469 | Jawwal cmpid=468
"""
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(
        url, headers={"User-Agent": "GooglePalestineG01-LocalProxy/1.0"}, method="GET"
    )
    try:
...
def main() -> None:
    ...
    print("=" * 56)
    print(" Google Palestine G01 (adid=232)")
    print(" http://%s:%s/?clickid=TEST" % (HOST, PORT))
    print(" Live: https://castmax-fun.com/psgog01?clickid={gclid}")
