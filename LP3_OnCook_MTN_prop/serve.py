#!/usr/bin/env python3
"""
Cameroon Dual Operator (Orange OneGaming + MTN OnCook) Propeller LP.
  python3 serve.py
...
HOST = "127.0.0.1"
PORT = 8100
ALLOWED_HOSTS = {"159.89.163.174", "168.144.122.72"}
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "CM-OnCook-LocalProxy/1.0"}, method="GET")
    try:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def proxy_cm(self, parsed: urllib.parse.ParseResult) -> None:
        ...
        api_host = (qs.get("host") or [""])[0].strip()
    ...
    def proxy_propeller(self, parsed: urllib.parse.ParseResult) -> None:
        ...
        payout = ((qs.get("payout") or ["100"])[0]).strip() or "100"
...
def main() -> None:
    ...
    print("Cameroon OnCook/OneGaming LP: http://%s:%s/" % (HOST, PORT))
