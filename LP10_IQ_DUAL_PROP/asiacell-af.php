...
if ($mcpuniqid === '') {
    $mcpuniqid = af_header_value($headerStr, 'uniqId');
}
if ($mcpuniqid === '') {
    $mcpuniqid = af_header_value($headerStr, 'UniqId');
}
...
if (is_array($parsed)) {
    ...
    if ($mcpuniqid === '' && !empty($parsed['uniqId'])) {
        $mcpuniqid = (string) $parsed['uniqId'];
    }
    if ($mcpuniqid === '' && !empty($parsed['UniqId'])) {
        $mcpuniqid = (string) $parsed['UniqId'];
    }
...
$antifraudid = trim($antifraudid);
$mcpuniqid = trim($mcpuniqid);
/* Never report MCP as a duplicate of AntiFrauduniqid */
if ($mcpuniqid !== '' && $antifraudid !== '' && $mcpuniqid === $antifraudid) {
    $mcpuniqid = '';
}
