#!/usr/bin/env python3
"""
Iraq 3-Operator LP (movstreamlite UI) local server (no PHP needed).
  python3 serve.py
...
PROPELLER = "https://ad.propellerads.com/conversion.php"
PROP_AID = "3898869"
PROP_TID = "154120"
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "IQ3op-LocalProxy/1.0"}, method="GET")
    try:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def proxy_propeller(self, parsed: urllib.parse.ParseResult) -> None:
        ...
        payout = ((qs.get("payout") or ["0.25"])[0]).strip() or "0.25"
        low = visitor_id.lower()
        if not visitor_id or visitor_id.startswith("local_") or low in ("clickid", "subid", "visitor_id"):
            self._send(400, json.dumps({"status": False, "msg": "Missing visitor_id"}).encode(), "application/json")
            return
        pb = PROPELLER + "?" + urllib.parse.urlencode({
            "aid": PROP_AID, "pid": "", "tid": PROP_TID,
...
def main() -> None:
    ...
    print("Iraq 3-Operator LP3 (movstreamlite): http://%s:%s/" % (HOST, PORT))
    print("Proxies: /zeen-api.php  /propeller-pb.php  /af-proxy.php")
    try:
