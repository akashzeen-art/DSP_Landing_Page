#!/usr/bin/env python3
"""
Oman Advertizer LP local server (no PHP needed).
  python3 serve.py
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "OmanAdvertizer-LocalProxy/1.0"}, method="GET")
    try:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def proxy_advertizer(self, parsed: urllib.parse.ParseResult) -> None:
        ...
        if (
            not clickid
            or clickid in ("${SUBID}", "{clickid}", "{CLICKID}")
...
def main() -> None:
    ...
    print("Oman Advertizer LP: http://%s:%s/" % (HOST, PORT))
