#!/usr/bin/env python3
"""
Botswana Orange Premium games Advertizer LP (Theme-496 + Zeen adnet cid=3198).
  python3 serve.py
...
def _fetch(url: str, timeout: int = 90) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "BW-Orange-PremiumGames-Adver-LocalProxy/1.0"}, method="GET")
    try:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def proxy_advertizer(self, parsed: urllib.parse.ParseResult) -> None:
        ...
        if (
            ...
            or clickid in ("{clickid}", "[[subid]]", "${SUBID}")
            or "{" in clickid
            or "[[" in clickid
        ):
        ...
        out = json.dumps({
            ...
            "clickid": clickid, "txn_id": txn_id, "amount": amount,
...
def main() -> None:
    ...
    print("BW Orange Premium games Advertizer LP: http://%s:%s/" % (HOST, PORT))
