#!/usr/bin/env python3
"""
Iraq 3-Operator GameX Google LP — fun-mediacontent.com
  python3 serve.py
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "IQ-GOG-LocalProxy/1.0"}, method="GET")
    try:
...
def main() -> None:
    ...
    print("Iraq GameX GOG LP: http://%s:%s/" % (HOST, PORT))
    print("Proxies: /zeen-api.php  /af-proxy.php")
    try:
