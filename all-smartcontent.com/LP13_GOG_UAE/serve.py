#!/usr/bin/env python3
"""
UAE Etisalat foodie Google LP (Holaa UI + Zeen adnet cid=3186).
  python3 serve.py
...
def _fetch(url: str, timeout: int = 90) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "UAE-Foodie-LocalProxy/1.0"}, method="GET")
    try:
...
def main() -> None:
    ...
    print("UAE foodie LP: http://%s:%s/" % (HOST, PORT))
