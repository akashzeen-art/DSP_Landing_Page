#!/usr/bin/env python3
"""
Oman Clickadu LP local server (no PHP needed).
  python3 serve.py
  http://127.0.0.1:8084/?clickid=TEST123&zoneid=1
"""
...
POSTBACK = "https://sconvtrk.com/conversion/c9a445f69b2775082add794af494a0a289412ae3/"
CLICKADU_AID = "307904"
_SSL = ssl.create_default_context()


def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "OmanClickadu-LocalProxy/1.0"}, method="GET")
    try:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def proxy_clickadu(self, parsed: urllib.parse.ParseResult) -> None:
        qs = urllib.parse.parse_qs(parsed.query, keep_blank_values=True)
        visitor_id = (
            (qs.get("visitor_id") or qs.get("clickid") or qs.get("subid") or [""])[0]
        ).strip()
        low = visitor_id.lower()
        if (
            not visitor_id
            or visitor_id in ("${SUBID}", "{subid}", "{SUBID}")
            or low in ("clickid", "subid", "visitor_id")
            or visitor_id.startswith("local_")
        ):
...
def main() -> None:
    ...
    print("Oman Clickadu LP: http://%s:%s/" % (HOST, PORT))
    print("Proxies: /zeen-api.php  /clickadu-pb.php")
    try:
