(function () {
  ...
  /**
   * Oman Omantel — ZD Game (Zeen Digital) + Clickadu
   * UI: Oman Propellar (StreamPulse)
   *
   * APIs (cid=3049):
   *   sendpin / verifypin / checkstatus
   * Portal: /Promo/Api/CPportal?cid=388
   * Price: 0.25 OMR/day — unsub: UNSUB IVID → 92149
   * Clickadu visitor_id = campaign ${SUBID} from clickid / subid
   */
  ...
  function isRealSubid(v) {
    if (!v) return false;
    var s = String(v).trim();
    if (!s) return false;
    var low = s.toLowerCase();
    if (s === "${SUBID}" || s === "{subid}" || s === "{SUBID}") return false;
  ...
  function initTracking() {
    ...
    var clickId =
      ...
      params.get("token") ||
      "";
    if (isRealSubid(clickId)) persist("click_id", clickId);
  ...
  function getClickId() {
    ...
    if (isRealSubid(existing)) return existing;
    var fallback =
  ...
  function getVisitorId() {
    var existing = track("click_id");
    return isRealSubid(existing) ? existing : "";
  }
  ...
  function fireClickaduPostback(done) {
    ...
    try {
      fetch("clickadu-pb.php?" + qs, { method: "GET", keepalive: true })
        .then(function (r) {
