#!/usr/bin/env python3
"""
KSA Mobily Playlive Propeller LP local server.
  python3 serve.py
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "KSA-Mobily-Playlive-LocalProxy/1.0"}, method="GET")
    try:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def proxy_propeller(self, parsed: urllib.parse.ParseResult) -> None:
        ...
        payout = ((qs.get("payout") or ["3"])[0]).strip() or "3"
...
def main() -> None:
    ...
    print("KSA Mobily Playlive LP: http://%s:%s/" % (HOST, PORT))
