#!/usr/bin/env python3
"""
Sri Lanka Dual Operator (Mobitel + Hutch) Propeller LP — movstreamlite UI.
  python3 serve.py
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "LKDual-LocalProxy/1.0"}, method="GET")
    try:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def proxy_propeller(self, parsed: urllib.parse.ParseResult) -> None:
        ...
        payout = (qs.get("payout") or ["10"])[0].strip() or "10"
...
def main() -> None:
    ...
    print("Sri Lanka Dual Propeller LP3 (movstreamlite): http://%s:%s/" % (HOST, PORT))
