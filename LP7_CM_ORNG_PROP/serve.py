#!/usr/bin/env python3
"""
Cameroon Orange OneGaming Propeller LP (LP7 bluescreen UI + CMcmp APIs).
  python3 serve.py
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "CM-Orange-LP7-LocalProxy/1.0"}, method="GET")
    try:
...
def main() -> None:
    ...
    print("Cameroon Orange OneGaming LP7: http://%s:%s/" % (HOST, PORT))
