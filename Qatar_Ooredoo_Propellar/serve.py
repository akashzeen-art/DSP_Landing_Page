#!/usr/bin/env python3
"""
Qatar Ooredoo Propeller LP local server (no PHP needed).
  python3 serve.py
  http://127.0.0.1:8086/?clickid=TEST123&zoneid=1
"""
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "QatarOoredoo-LocalProxy/1.0"}, method="GET")
    try:
...
def main() -> None:
    ...
    print("Qatar Ooredoo Propeller LP: http://%s:%s/" % (HOST, PORT))
