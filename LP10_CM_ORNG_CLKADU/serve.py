#!/usr/bin/env python3
"""
Cameroon Orange OneGaming Clickadu LP (Theme-496 UI + CMcmp cid=502).
  python3 serve.py
...
CLICKADU = "https://sconvtrk.com/conversion/c9a445f69b2775082add794af494a0a289412ae3/"
_SSL = ssl.create_default_context()


def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "CM-Orange-Clkadu-LocalProxy/1.0"}, method="GET")
    try:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def proxy_clickadu(self, parsed: urllib.parse.ParseResult) -> None:
        ...
        if (not visitor_id or visitor_id.startswith("local_") or
                visitor_id in ("${SUBID}", "{clickid}", "{subid}", "clickid", "subid")):
        ...
        out = json.dumps({
            ...
            "visitor_id": visitor_id, "aid": "307904",
...
def main() -> None:
    ...
    print("Cameroon Orange OneGaming Clickadu: http://%s:%s/" % (HOST, PORT))
