#!/usr/bin/env python3
"""
Iraq Asiacell FunPlus / Gamifya Advertizer LP.
  python3 serve.py
...
def _fetch(url: str, timeout: int = 30, want_headers: bool = False) -> tuple[int, bytes, dict]:
    req = urllib.request.Request(url, headers={"User-Agent": "IQ-FunPlus-Advertizer-LocalProxy/1.0"}, method="GET")
    try:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def proxy_advertizer(self, parsed: urllib.parse.ParseResult) -> None:
        ...
        amount = ((qs.get("amount") or ["360"])[0]).strip() or "360"
...
def main() -> None:
    ...
    print("Iraq Asiacell FunPlus Advertizer LP: http://%s:%s/" % (HOST, PORT))
