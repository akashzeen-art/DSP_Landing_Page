#!/usr/bin/env python3
"""
LK Mobitel ZD X Gamez Advertizer LP local server.
  python3 serve.py
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "LK-Mobitel-Advertizer-LocalProxy/1.0"}, method="GET")
    try:
...
def main() -> None:
    ...
    print("LK Mobitel ZD X Gamez Advertizer LP: http://%s:%s/" % (HOST, PORT))
