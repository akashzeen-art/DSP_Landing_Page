#!/usr/bin/env python3
"""
UAE Etisalat Gamespro Propeller LP local server.
  python3 serve.py
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "UAE-Etisalat-Gamespro-LocalProxy/1.0"}, method="GET")
    try:
...
def main() -> None:
    ...
    print("UAE Etisalat Gamespro LP: http://%s:%s/" % (HOST, PORT))
