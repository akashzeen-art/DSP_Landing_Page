(function () {
   ...
   * UI: FileShareDownload Lite (hitzmobi) style
   *
   * Campaign:
  ...
  var t = {
    en: {
      ...
      titleTxt: "Access full content",
      page1txt: "Follow the instructions to access full content",
      nextBtn: "Next",
      pnTitle: "Step 1/2: Enter your mobile number",
      ...
      pinTitle: "Step 2/2: Verify your mobile number with OTP",
    ...
    ar: {
      ...
      titleTxt: "الوصول إلى المحتوى الكامل",
      page1txt: "اتبع التعليمات للوصول إلى المحتوى الكامل",
      nextBtn: "التالي",
      pnTitle: "الخطوة 1/2: ادخل رقم جوالك",
      ...
      pinTitle: "الخطوة 2/2: التحقق من رقم الهاتف المحمول باستخدام OTP",
  ...
  function toggleValidUi(ok, wrapSel, iconSel) {
    var check = document.querySelector(wrapSel + " .sidebtncheck");
    var icon = document.querySelector(iconSel);
    var arrows = document.querySelectorAll(wrapSel + " .arrow");
    if (check) check.style.display = ok ? "block" : "none";
    if (icon) {
      if (ok) icon.classList.remove("icn_wiggle");
      else icon.classList.add("icn_wiggle");
    }
    arrows.forEach(function (a, i) {
      if (ok) {
        a.classList.add("bounceAlpha-play");
        if (i === 0) a.classList.add("bounceAlpha-delay");
      } else {
        a.classList.remove("bounceAlpha-play");
        a.classList.remove("bounceAlpha-delay");
      }
    });
  }
  ...
  if (mForm) {
    ...
    mInput.addEventListener("input", function () {
      ...
      showError("");
      var ok = msisdnFormat.test(mInput.value);
      toggleValidUi(ok, ".btnpn", ".inputicon");
    });
  ...
  if (pForm) {
    ...
    pInput.addEventListener("input", function () {
      ...
      toggleValidUi(ok, ".btnpin", "#pbox .inputicon");
    });
