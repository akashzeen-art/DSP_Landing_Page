#!/usr/bin/env python3
"""
Kuwait 3-Operator CuriousCubs LP — fun-mediacontent.com
  python3 serve.py
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "KW-GOG-LocalProxy/1.0"}, method="GET")
    try:
...
def main() -> None:
    ...
    print("Kuwait 3-Operator GOG LP: http://%s:%s/" % (HOST, PORT))
