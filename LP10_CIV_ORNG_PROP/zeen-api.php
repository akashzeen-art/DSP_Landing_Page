<?php
/**
 * Proxy for CIV Orange CIVOrgcmp PIN APIs (11Barre cid=503).
 * GET zeen-api.php?host=159.89.163.174&path=sendPIN&cid=503&msisdn=225...&ip=...
 */
...
$url = 'http://' . $host . '/prod/CIVOrgcmp/' . $path . ($query !== '' ? ('?' . $query) : '');
...
if (function_exists('curl_init')) {
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_USERAGENT => 'CIV-Orange-11Barre/1.0',
    ]);
    $body = curl_exec($ch);
    ...
    $ctx = stream_context_create([
        'http' => [
            'timeout' => 30,
            'ignore_errors' => true,
            'header' => "User-Agent: CIV-Orange-11Barre/1.0\r\n",
        ],
    ]);
