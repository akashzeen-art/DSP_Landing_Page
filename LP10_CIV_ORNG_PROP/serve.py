#!/usr/bin/env python3
"""
Côte d'Ivoire Orange 11Barre Propeller LP (Theme-496 + CIVOrgcmp cid=503).
  python3 serve.py
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "CIV-Orange-11Barre-LocalProxy/1.0"}, method="GET")
    try:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def proxy_civ(self, parsed: urllib.parse.ParseResult) -> None:
        ...
        url = "http://%s/prod/CIVOrgcmp/%s%s" % (api_host, api_path, ("?" + query) if query else "")
...
def main() -> None:
    ...
    print("CIV Orange 11Barre Propeller LP: http://%s:%s/" % (HOST, PORT))
