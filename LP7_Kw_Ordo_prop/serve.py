#!/usr/bin/env python3
"""
Kuwait Ooredoo Premium Movie Propeller LP local server.
  python3 serve.py
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "KwPremiumMovie-LocalProxy/1.0"}, method="GET")
    try:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def proxy_propeller(self, parsed: urllib.parse.ParseResult) -> None:
        ...
        payout = ((qs.get("payout") or ["0.8"])[0]).strip() or "0.8"
...
def main() -> None:
    ...
    print("KW Ooredoo Premium Movie LP: http://%s:%s/" % (HOST, PORT))
