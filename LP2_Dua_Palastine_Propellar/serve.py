#!/usr/bin/env python3
"""
Palestine Dual Operator Propeller LP (flashdownloadlite UI) — local server.
  python3 serve.py
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "PSDual-LP3-LocalProxy/1.0"}, method="GET")
    try:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def proxy_propeller(self, parsed: urllib.parse.ParseResult) -> None:
        ...
        payout = (qs.get("payout") or ["1.16"])[0].strip() or "1.16"
        if not visitor_id or visitor_id in ("${SUBID}", "{clickid}") or visitor_id.startswith("local_"):
...
def main() -> None:
    ...
    print("Palestine Dual LP3 (flashdownloadlite): http://%s:%s/" % (HOST, PORT))
