#!/usr/bin/env python3
"""
LK Mobitel ZD X Gamez Propeller LP local server.
  python3 serve.py
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "LK-Mobitel-LocalProxy/1.0"}, method="GET")
    try:
...
def main() -> None:
    ...
    print("LK Mobitel ZD X Gamez LP: http://%s:%s/" % (HOST, PORT))
