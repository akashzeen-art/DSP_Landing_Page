#!/usr/bin/env python3
"""
Palestine Dual Operator Propeller LP local server (no PHP needed).
  python3 serve.py
...
def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "PSDual-LocalProxy/1.0"}, method="GET")
    try:
...
def main() -> None:
    ...
    print("Palestine Dual Propeller LP: http://%s:%s/" % (HOST, PORT))
