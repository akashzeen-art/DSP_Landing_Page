...
if ($path === '') {
    ...
    echo json_encode(['response' => 'FAIL', 'errorMessage' => 'Missing path']);
    exit;
}

// Portal is an HTML redirect destination — send user there
if ($path === 'portal') {
    $params = $_GET;
    unset($params['path']);
    $url = 'http://64.225.87.221/adpoke/cnt/inapp/portal?' . http_build_query($params);
