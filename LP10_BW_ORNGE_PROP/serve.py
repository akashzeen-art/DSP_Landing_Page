#!/usr/bin/env python3
"""
Botswana Orange Premium games Propeller LP (Theme-496 + Zeen adnet cid=3183).
  python3 serve.py
...
def _fetch(url: str, timeout: int = 90) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "BW-Orange-PremiumGames-LocalProxy/1.0"}, method="GET")
    try:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def proxy_propeller(self, parsed: urllib.parse.ParseResult) -> None:
        ...
        payout = ((qs.get("payout") or ["3.56"])[0]).strip() or "3.56"
...
def main() -> None:
    ...
    print("BW Orange Premium games Propeller LP: http://%s:%s/" % (HOST, PORT))
