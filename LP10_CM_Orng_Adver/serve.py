...
ADVERTIZER_PB = "http://postback.advertizer.com/pb.php"
_SSL = ssl.create_default_context()


def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "CM-Orange-Adver-LocalProxy/1.0"}, method="GET")
    try:
...
def _is_real_clickid(v: str) -> bool:
    if not v:
        return False
    low = v.lower()
    if v.startswith("local_"):
        return False
    if low in ("clickid", "subid", "visitor_id", "undefined", "null"):
        return False
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def proxy_advertizer(self, parsed: urllib.parse.ParseResult) -> None:
        qs = urllib.parse.parse_qs(parsed.query, keep_blank_values=True)
        clickid = ((qs.get("clickid") or qs.get("click_id") or qs.get("subid") or [""])[0]).strip()
        amount = ((qs.get("amount") or ["1"])[0]).strip() or "1"
        txn_id = ((qs.get("txn_id") or [clickid])[0]).strip() or clickid
        if not _is_real_clickid(clickid):
            self._send(400, json.dumps({"status": False, "msg": "Missing or invalid clickid"}).encode(), "application/json")
            return
        pb = ADVERTIZER_PB + "?" + urllib.parse.urlencode({
            "clickid": clickid,
            "txn_id": txn_id,
            "amount": amount,
