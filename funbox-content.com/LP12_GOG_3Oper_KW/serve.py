#!/usr/bin/env python3
"""
Kuwait 3-Operator ZD GameMedias LP — funbox-content.com
  python3 serve.py
...
def _fetch(url: str, timeout: int = 90) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "KW-3Oper-GOG-LocalProxy/1.0"}, method="GET")
    try:
...
def main() -> None:
    ...
    print("KW 3-Op ZD GameMedias LP: http://%s:%s/" % (HOST, PORT))
