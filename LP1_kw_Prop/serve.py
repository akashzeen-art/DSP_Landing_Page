#!/usr/bin/env python3
"""
Kuwait 3-Operator Propeller LP (loaderlite UI) local server.
  python3 serve.py
...
AF_PREFIX = "http://40.172.132.15/kwstc/mystrycenter/antifraud.php"
_SSL = ssl.create_default_context()


def _fetch(url: str, timeout: int = 30) -> tuple[int, bytes]:
    req = urllib.request.Request(url, headers={"User-Agent": "KW3op-LocalProxy/1.0"}, method="GET")
    try:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def proxy_propeller(self, parsed: urllib.parse.ParseResult) -> None:
        ...
        payout = ((qs.get("payout") or ["0.2"])[0]).strip() or "0.2"
    ...
    def proxy_af(self, parsed: urllib.parse.ParseResult) -> None:
        ...
        if not url or not url.startswith(AF_PREFIX):
            self._send(400, json.dumps({"status": False, "msg": "Invalid url"}).encode(), "application/json")
            return
        code, body = _fetch(url, timeout=15)
        self._send(code if 100 <= code <= 599 else 200, body, "text/plain")


def main() -> None:
    server = ThreadingHTTPServer((HOST, PORT), Handler)
    print("Kuwait 3-Operator Propeller LP1 (loaderlite): http://%s:%s/" % (HOST, PORT))
