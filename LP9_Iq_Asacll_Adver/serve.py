#!/usr/bin/env python3
"""
Iraq Asiacell Gamifya Advertizer LP (Theme-340 UI).
  python3 serve.py
...
def _fetch(url: str, timeout: int = 30, want_headers: bool = False) -> tuple[int, bytes, dict]:
    req = urllib.request.Request(url, headers={"User-Agent": "IQ-Gamifya-Advertizer-LocalProxy/1.0"}, method="GET")
    try:
...
def main() -> None:
    ...
    print("Iraq Asiacell Gamifya Advertizer LP: http://%s:%s/" % (HOST, PORT))
    print("Proxies: /iq-api.php  /asiacell-af.php  /advertizer-pb.php")
    try:
